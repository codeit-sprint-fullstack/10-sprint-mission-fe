...
function setupAuthenticationForm(rootEl) {
  ...
  const nicknameField = signupField
    ? form.querySelector(".authentication-nickname")
    : null; // 회원가입에 있는 필드 찾기

  const passwordGroups = form.querySelectorAll(".authentication-password");
  const passwordConfirmField =
    signupField && passwordGroups.length > 1 ? passwordGroups[1] : null; // 비밀번호 확인 필드 찾기
  ...
  passwordConfirmField
    ?.querySelector('input[type="password"]')
    ?.setAttribute("required", "true"); // 각 필드의 input에 required 속성 추가 (required="true")
  ...
  form.addEventListener("submit", (e) => {
    ...
    const validNickname = nicknameField
      ? checkNicknameInput(nicknameField)
      : true;
    ...
    const isFormValid =
      validEmail && validPw && validNickname && validPwConfirm;

    changeButtonLooking(submitButton, !isFormValid);
    if (!isFormValid) return;

    const typeEmail = cleanInput(
      emailField.querySelector("input")?.value || ""
    );
    const typePassword = cleanInput(
      passwordField.querySelector('input[type="password"], input[type="text"]')
        ?.value || ""
    );
  ...
  form
    .querySelectorAll(".authentication-password .visibility-icon")
    .forEach((btn) => {
