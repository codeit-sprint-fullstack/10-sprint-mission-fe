...
function login_btn_disabled() {
  ...
  const isValid =
    !login_pw.classList.contains("error") &&
    !login_id.classList.contains("error") &&
    login_pw.value !== "" &&
    login_id.value !== "";
  if (isValid) {
...
const panda_btn_login_click_event = document.querySelector(
  "#panda_btn_login_02"
);
...
function findEmail(e) {
  ...
  const user = USER_DATA.find(
    (el) => login_id_Value === el.email && login_pw_Value === el.password
  );
