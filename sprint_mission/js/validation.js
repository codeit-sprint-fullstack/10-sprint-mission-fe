...
export const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/; // 이메일 정규식

export const cleanInput = (str) => (str || "").trim(); // null/undefined 에러를 방지하고, 앞뒤 공백도 자동으로 제거

export const minimumLetter = (str, min) => cleanInput(str).length >= min; // 최소 글자수 검사

//============================== error 표시 & 제거 함수 ==============================//

export function showErrorMessage(fieldEl, message) {
  if (!fieldEl) return;
...
export function eraseErrorMessage(fieldEl) {
  if (!fieldEl) return;
...
export function formHasError(formEl) {
  const alreadyHasError = formEl.querySelector(".is-error") !== null;
  const hasEmptyInput = Array.from(
    formEl.querySelectorAll("input[required]")
  ).some((input) => cleanInput(input.value) === "");
  return alreadyHasError || hasEmptyInput;
} // 폼에 에러가 있거나 input이 비었는지 확인하는 함수

export function changeButtonLooking(buttonEl, disabled) {
  if (!buttonEl) return;
  buttonEl.disabled = Boolean(disabled);
  buttonEl.style.backgroundColor = disabled
    ? "var(--Secondary-400)"
    : "var(--Primary-100)";
...
export function checkEmailInput(fieldEl) {
  const input = fieldEl?.querySelector("input");
...
export function checkPasswordInput(fieldEl) {
...
export function checkNicknameInput(fieldEl) {
  const input = fieldEl?.querySelector("input");
...
export function checkConfirmPasswordInput(fieldEl, passwordFieldEl) {
  const input = fieldEl?.querySelector(
    'input[type="password"], input[type="text"]'
  );
  const pwInput = passwordFieldEl?.querySelector(
    'input[type="password"], input[type="text"]'
  );
